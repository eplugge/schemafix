...
CREATE TABLE `android_work_restriction` (
  ...
  CONSTRAINT `android_work_restriction_ibfk_1` FOREIGN KEY (`app_catalog_id`) REFERENCES `mi_app_catalog` (`id`)
);
...
CREATE TABLE `mi_app_reviews` (
  ...
  CONSTRAINT `mi_app_reviews_ibfk_2` FOREIGN KEY (`app_id`) REFERENCES `mi_app_catalog` (`id`)
);
...
CREATE TABLE `mi_bsp_app` (
  ...
  `type` varchar(10) COLLATE utf8_unicode_ci NOT NULL COMMENT 'The app type'
  ...
  CONSTRAINT `mi_bsp_app_ibfk_1` FOREIGN KEY (`catalog_id`) REFERENCES `mi_app_catalog` (`id`)
);
...
CREATE TABLE `override_app_bundle` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT 'primary key or id which is auto incremented'
  ...
  `modified_at` datetime NOT NULL COMMENT 'date last modified'
  PRIMARY KEY (`id`)
  UNIQUE KEY `idx_override_appbundle_u01` (`property`)
);
