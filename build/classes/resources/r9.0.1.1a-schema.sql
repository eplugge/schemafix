...
CREATE TABLE `android_work_restriction` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT 'primary key for the android_work_restriction'
  `app_catalog_id` bigint(20) NOT NULL COMMENT 'foreign key for the the mi_app_catalog table'
  ...
  PRIMARY KEY (`id`)
  UNIQUE KEY `android_work_restriction_udx_cat_id_restriction_key` (`app_catalog_id`,`restriction_key`)
);
...
CREATE TABLE `app_inventory_details` (
  ...
  UNIQUE KEY `app_inventory_id_key` (`app_inventory_id`)
);
...
CREATE TABLE `mi_app_catalog` (
  ...
  `install_name` varchar(512) COLLATE utf8_unicode_ci DEFAULT NULL
  ...
  `modified_at` bigint(20) DEFAULT NULL
  `trash` char(1) COLLATE utf8_unicode_ci DEFAULT 'f'
  `web_app_uri` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL
  `developer_name` varchar(255) COLLATE utf8_unicode_ci DEFAULT ''
  ...
  `install_size` bigint(20) DEFAULT NULL
  `catalog_type` varchar(10) COLLATE utf8_unicode_ci DEFAULT 'APP'
  PRIMARY KEY (`id`)
...
CREATE TABLE `mi_app_catalog_entry` (
  ...
  KEY `mi_app_catalog_entry_idx_prop_value` (`property`,`value`)
...
CREATE TABLE `mi_app_fingerprints` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT
  `catalog_id` bigint(20) NOT NULL
  `checksum` varchar(512) COLLATE utf8_unicode_ci DEFAULT NULL
  PRIMARY KEY (`id`)
...
CREATE TABLE `mi_app_inventory` (
  ...
  `version_long` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL
  `version_short` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL
  `rollup_id` bigint(20) DEFAULT NULL
  `catalog_type` varchar(10) COLLATE utf8_unicode_ci DEFAULT 'APP'
  PRIMARY KEY (`id`)
  KEY `FK4393A1554E1E1744` (`catalog_id`)
  KEY `idx_mi_app_inventory_checksum` (`checksum`)
  KEY `idx_mi_app_inventory_bundle` (`bundle`)
  KEY `idx_mi_app_inventory_n02` (`install_name`(255),`checksum`(255),`platform`,`catalog_id`,`created_at`,`name`,`version`(128))
...
CREATE TABLE `mi_app_wp8app` (
  ...
  `trash` char(1) COLLATE utf8_unicode_ci DEFAULT 'f'
  `product_family_name` varchar(64) COLLATE utf8_unicode_ci DEFAULT NULL
  PRIMARY KEY (`id`)
);
...
CREATE TABLE `mi_bsp_app` (
  ...
  `catalog_id` bigint(20) NOT NULL COMMENT 'foreign key which refer to id in mi_app_catalog'
  ...
  `license_type` varchar(10) COLLATE utf8_unicode_ci NOT NULL COMMENT 'BSP license type'
  ...
  `available_seats` bigint(20) DEFAULT NULL COMMENT 'Number of available seats remaining for an applcation when importing to VSP'
  ...
  `package_set_id` varchar(64) COLLATE utf8_unicode_ci NOT NULL COMMENT 'BSP app package set id'
  `content_details_json` text COLLATE utf8_unicode_ci COMMENT 'A JSON string which includes license and app package details'
  `created_at` datetime NOT NULL COMMENT 'When the row is created'
  `updated_at` datetime NOT NULL COMMENT 'When the row is updated'
  PRIMARY KEY (`id`)
  KEY `catalog_id` (`catalog_id`)
  CONSTRAINT `mi_bsp_app_ibfk_1` FOREIGN KEY (`catalog_id`) REFERENCES `app_catalog` (`id`)
);
CREATE TABLE `mi_bsp_license_usage` (
  `id` int(11) NOT NULL AUTO_INCREMENT COMMENT 'Primary key for the mi_bsp_license_usage'
  `bsp_app_id` int(11) NOT NULL COMMENT 'The BSP App id'
  `user_id` bigint(20) NOT NULL COMMENT 'The user who claimed the BSP offline license'
  `device_id` bigint(20) NOT NULL COMMENT 'The device which installed teh BSP offline license'
  `created_at` datetime NOT NULL COMMENT 'When offline license is claimed'
  `reclaimed_at` datetime DEFAULT NULL COMMENT 'When offline license is reclaimed'
  PRIMARY KEY (`id`)
  KEY `bsp_app_id` (`bsp_app_id`)
  KEY `user_id` (`user_id`)
  KEY `device_id` (`device_id`)
...
CREATE TABLE `mi_client_app_inventory` (
  ...
  `bmw_failures` varchar(2048) COLLATE utf8_unicode_ci DEFAULT NULL
  ...
  PRIMARY KEY (`id`)
  KEY `mcai_inventory_id` (`inventory_id`)
  CONSTRAINT `mcai_inventory_id` FOREIGN KEY (`inventory_id`) REFERENCES `mi_app_inventory` (`id`)
);
...
CREATE TABLE `mi_webapp_app_setting_assoc` (
  `web_app_id` bigint(20) NOT NULL
  `app_setting_id` bigint(20) NOT NULL
  PRIMARY KEY (`app_setting_id`)
  UNIQUE KEY `web_app_id` (`web_app_id`)
  UNIQUE KEY `web_app_id_2` (`web_app_id`,`app_setting_id`)
  CONSTRAINT `mi_webapp_app_setting_assoc_ibfk_1` FOREIGN KEY (`web_app_id`) REFERENCES `mi_app_catalog` (`id`)
  CONSTRAINT `mi_webapp_app_setting_assoc_ibfk_2` FOREIGN KEY (`app_setting_id`) REFERENCES `mi_app_setting` (`id`)
);
...
CREATE TABLE `q_activation_request` (
  ...
  PRIMARY KEY (`id`)
  KEY `idx_q_activation_request_state_type_prio` (`state`,`type`,`priority`)
  KEY `idx_activation_request_check_duplicate` (`state`,`type`,`device_id`,`json_md5`)
);
